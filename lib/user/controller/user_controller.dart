...
class UserController with ChangeNotifier  {
      ...
      if(codigo != user.code){
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                ...
                content: Row(
                    ...
                    children: [
                        Text('Código errado, insira o código correto!', style: TextStyle(fontSize: 10),),
                        Icon(Icons.error)
                    ],
                )));
      }else{
      ...
      if (e.code == 'weak-password') {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                ...
                content: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                        Text('Senha fraca, tente inserir uma senha mais forte!', style: TextStyle(fontSize: 10),),
                        Icon(Icons.error)
                    ],
                )));
       ...
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                ...
                content: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                        Text('Ocorre um erro durante o cadastro', style: TextStyle(fontSize: 10),),
                        Icon(Icons.error)
                    ],
                )));
    }
    ...
    void login(context, emailController, senhaController) async{
      ...
      try {
        ...
        findUserByEmail(emailController);
        ...
        if (e.code == 'user-not-found') {
         ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                ...
                content: Row(
                   mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                        Text('Usuário não encontrado, caso não tenha feito a conta clique em cadastro!'),
                        Icon(Icons.error)
                    ],
                )));
