...
Future<List<Product>> getProduct() async {
  ...
  try {
    ...
    productsList = querySnapshot.docs.map((document) {
      return Product.fromSnapshot(document);
    })
    .where((document) => document.id != 'SaAf7kOeu93DulrFYQvv')
    .toList();
  ...
  void addProduct(context, Product product)async{
  ...
  addItem(context, String name, int quantity) async {
  ...
  if (product != null) {
   ...
   void editItem(context, Product product)async{
   final db = FirebaseFirestore.instance;
   final querySnapshot = await db.collection("products").where("name", isEqualTo: product.name).get();
   if(querySnapshot.docs.isNotEmpty){
      String productID = querySnapshot.docs.first.id;
      db.collection('products').doc(productID).update(product.toMap())
      .then((_){
